...
describe("completed trip editing option", () => {
  ...
  it("automatically completes elapsed trips without exposing the lifecycle RPC to anonymous callers", () => {
    const elapsedMigration = readFileSync(
      "supabase/migrations/202608110002_auto_complete_elapsed_trips.sql",
      "utf8",
    );

    expect(elapsedMigration).toContain(
      "create or replace function public.sync_elapsed_trip_lifecycle()",
    );
    expect(elapsedMigration).toContain("trip.end_date <");
    expect(elapsedMigration).toContain("allow_completed_edits = true");
    expect(elapsedMigration).toContain("from public, anon");
    expect(elapsedMigration).toContain("to authenticated");
  });
