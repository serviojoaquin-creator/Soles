...
## Cierre automático de viajes vencidos

Estado: aplicada en Supabase y publicada el 11 de agosto de 2026.

- Al entrar al dashboard se sincronizan los viajes de los que la persona es integrante: si el día local de regreso ya pasó, pasan de `planning` o `active` a `completed` y se muestran en Recuerdos.
- La migración también regulariza los viajes vencidos que ya existían, sin modificar membresía ni permisos de edición.
- Verificación remota: `Escapada a San Martin`, cuyo regreso fue el 4 de agosto, quedó con estado `completed` y ediciones habilitadas.
# Progreso de Soles
